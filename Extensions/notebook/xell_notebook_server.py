#!/usr/bin/env python3
"""
Xell Notebook Server
Serves the notebook GUI and manages the Xell kernel process.
Usage: python3 notebook_server.py [notebook.nxel] [--port PORT]
"""
...
# Find the xell binary — check PATH first, then relative locations
XELL_BINARY = shutil.which("xell")
if not XELL_BINARY:
    candidates = [
        os.path.join(SCRIPT_DIR, "..", "xell"),
        os.path.join(SCRIPT_DIR, "..", "build", "xell"),
        os.path.join(SCRIPT_DIR, "..", "..", "build", "xell"),
        os.path.join(SCRIPT_DIR, "..", "..", "bin", "xell"),
        "/usr/local/bin/xell",
    ]
    for c in candidates:
        if os.path.isfile(c) and os.access(c, os.X_OK):
            ...
            break
if not XELL_BINARY:
    XELL_BINARY = "xell"  # hope it's in PATH
DEFAULT_PORT = 8888
...
class KernelManager:
    """Manages the Xell kernel subprocess."""
...
def load_notebook(filepath):
    """Load a .nxel notebook file."""
    with open(filepath, "r") as f:
        return json.load(f)


def save_notebook(filepath, notebook):
    """Save a .nxel notebook file."""
            ...
            if current_notebook:
                ...
                if not filepath:
                    filepath = os.path.join(NOTEBOOK_DIR,
                                          current_notebook["metadata"].get("title", "Untitled") + ".nxel")
...
def main():
    ...
    while i < len(args):
        if args[i] == "--port" and i + 1 < len(args):
            port = int(args[i + 1])
            i += 2
        elif args[i].endswith(".nxel"):
            notebook_file = os.path.abspath(args[i])
            i += 1
        else:
            i += 1

    if not os.path.exists(XELL_BINARY):
        print(f"Error: Xell binary not found at {XELL_BINARY}")
