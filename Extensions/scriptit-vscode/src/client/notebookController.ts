// ═══════════════════════════════════════════════════════════
// ScriptIt Notebook Controller (Kernel)
// ═══════════════════════════════════════════════════════════
//
// Manages the ScriptIt kernel subprocess and executes
...
export class ScriptItNotebookController {
    readonly controllerId = 'scriptit-kernel';
    readonly notebookType = 'scriptit-notebook';
    readonly label = 'ScriptIt Kernel';
    readonly supportedLanguages = ['scriptit'];
    ...
    private async ensureKernel(): Promise<boolean> {
        ...
        try {
            this.kernelProcess = spawn(scriptitPath, ['--kernel'], {
            ...
            if (readyMsg.status !== 'kernel_ready') {
                vscode.window.showErrorMessage(`ScriptIt kernel failed to start: ${readyLine}`);
                return false;
            }
            ...
            console.log(`[ScriptIt Kernel] Started (PID ${this.kernelProcess.pid})`);
            return true;
        } catch (err: unknown) {
            const msg = err instanceof Error ? err.message : String(err);
            vscode.window.showErrorMessage(
                `Failed to start ScriptIt kernel: ${msg}\n\nMake sure '${scriptitPath}' is installed.`
            );
            return false;
        }
    }
    ...
    private interrupt(_notebook: vscode.NotebookDocument): void {
        ...
        vscode.window.showInformationMessage('ScriptIt kernel interrupted and reset.');
    }
}
